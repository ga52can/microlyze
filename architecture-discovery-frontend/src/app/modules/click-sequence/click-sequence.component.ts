...
@ngComponent({
    templateUrl: 'click-sequence.component.html'
})
export class ClickSequenceComponent implements OnInit {

    @ViewChild('clicksequenceCanvasComponent') clicksequenceCanvasComponent: ClickSequenceCanvasComponent;
    ...
    public renameActivity() {
        ...
        this.clicksequenceCanvasComponent.renameActivity(this.nextRenameActivity, this.nextRenameActivityName);
    ...
    public renameProcess() {
        ...
        this.unsetCellRelations(this.clicksequenceCanvasComponent.getStartCell());
        this.saveRequired = true;
    ...
    public deleteProcess() {
        ...
        for (index = 0; index < this.processes.length; ++index) {
            if (this.processes[index] === this.selectedProcess) {
                ...
                for(let cell of this.clicksequenceCanvasComponent.getGraph().getCells()) {
    ...
    public saveProcess() {
        ...
        const graph = this.clicksequenceCanvasComponent.getGraph();
                ...
                this.revisionService.createRevisionIfRequired(this.selectedProcess).then((process) => {
                    ...
                    this.saveProcessRemovedLinks(graph).then(() => {
                        this.saveProcessLinks(graph.getLinks()).then(() => {
                            this.selectedProcess.setAnnotation(this.graphAnnotation, JSON.stringify(this.clicksequenceCanvasComponent.getGraph().toJSON()));
    ...
    public setCurrentProcess(process: Revision) {
        ...
        this.clicksequenceCanvasComponent.initGraph((stringGraph) ? JSON.parse(stringGraph) : null);
        this.saveRequired = false;
        this.deletedCells = new Array<joint.dia.Cell>();
        this.initialRelations = new Array<number>();
        for (let link of this.clicksequenceCanvasComponent.getGraph().getLinks()) {
            let relationId = link.get('model.relation.id');
            if (!isNaN(relationId))
    ...
    public createActivities() {
        ...
        for (let i = activities.length - 1; i >= 0; --i) {
            this.clicksequenceCanvasComponent.createActivity(activities[i]);
        }
        this.createActivityModal.hide();
    }

    private unsetCellRelations(cell: joint.dia.Cell) {
        for(let link of this.clicksequenceCanvasComponent.getGraph().getConnectedLinks(cell))
