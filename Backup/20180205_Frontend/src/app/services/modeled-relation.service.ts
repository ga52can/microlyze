...
export class ModeledRelationService extends AnnotationServiceBase {
  ...
  public createModeledRelation(relation: ModeledRelation): Promise<ModeledRelation> {
    return new Promise<ModeledRelation>((resolve, reject) => {
      ...
      this.http.post<Response>(this.apiUrl + endpoint, {callerId:relation.caller, calleeId:relation.callee},{headers: this.headers} ).map(response => ModeledRelation.deserialize(response)).subscribe(remoteModeledRelation => {
  ...
  public closeModeledRelation(relation: ModeledRelation): Promise<ModeledRelation> {
    return new Promise<ModeledRelation>((resolve, reject) => {
      ...
      this.http.delete<Response>(this.apiUrl + endpoint, {headers: this.headers}).subscribe(remoteModeledRelation => {
        resolve(relation);
      });
    })
  }
